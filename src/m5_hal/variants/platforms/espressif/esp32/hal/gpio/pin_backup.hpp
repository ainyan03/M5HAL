...
class PinBackup {
public:
    using gpio_number_t = ::m5::hal::v2::types::gpio_number_t;
    ...
    void backup()
    {
        _in_func_num = -1;
        _captured    = false;
        if (_pin_num < 0 || ::m5::hal::v2::types::extractSlot(_pin_num) != 0) {
            return;  // invalid or non-MCU (expander) pin -> nothing to save
        }
        const size_t pin = ::m5::hal::v2::types::extractLocalPin(_pin_num);
    ...
    void restore() const
    {
        if (!_captured || _pin_num < 0 || ::m5::hal::v2::types::extractSlot(_pin_num) != 0) {
            return;
        }
        const size_t pin = ::m5::hal::v2::types::extractLocalPin(_pin_num);
        if (pin >= SOC_GPIO_PIN_COUNT) {
...
// the `m5::hal::v2::gpio` flat injection (the offer scan picks one Port / GPIO
...
// GPIO HAL provider, `m5::hal::v2::gpio::PinBackup` stays reachable.
//
...
using ::m5::variants::platforms::espressif::esp32::hal::v2::gpio::PinBackup;
using ::m5::variants::platforms::espressif::esp32::hal::v2::gpio::ScopedPinBackup;
}  // namespace m5::hal::v2::gpio
