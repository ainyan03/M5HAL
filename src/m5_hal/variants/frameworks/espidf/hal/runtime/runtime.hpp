...
// runtime kind for the ESP-IDF framework variant: time functions only
// (millis/micros via esp_timer, delayMs via vTaskDelay, delayUs via
// esp_rom_delay_us). Mutex and Task are provided by the FreeRTOS
// framework variant. Authoritative contract: spec/design/runtime.md.
...
namespace m5::variants::frameworks::espidf::hal::v2::runtime {
...
inline void delayMs(uint32_t ms)
{
    ...
    vTaskDelay(::m5::hal::v2::detail::delayMsToTicksAtLeast(ms));
}
