// SPDX-License-Identifier: MIT
#ifndef M5_HAL_VARIANTS_FRAMEWORKS_ESPIDF_HAL_I2C_BACKEND_MASTER_WRITE_BUFFER_INL
#define M5_HAL_VARIANTS_FRAMEWORKS_ESPIDF_HAL_I2C_BACKEND_MASTER_WRITE_BUFFER_INL

#include "../../../../../hal/v2/data.hpp"
#include "../../../../../hal/v2/error.hpp"
#include "../../../../../hal/v2/memory/allocator.hpp"

#include <algorithm>
#include <cstring>

namespace m5::variants::frameworks::espidf::hal::v2::i2c::detail {

class TempWriteBuffer {
public:
    TempWriteBuffer() = default;

    ::m5::hal::v2::result_t<void> build(::m5::hal::v2::data::ConstDataSpan prefix, ::m5::hal::v2::data::Source* src,
                                        size_t tx_len)
    {
        ::m5::hal::v2::data::ConstDataSpan first{};
        if (src != nullptr && tx_len > 0 && !src->eof()) {
            auto peeked = src->peek(tx_len);
            if (!peeked.has_value()) {
                return m5::stl::make_unexpected(peeked.error());
            }
            first = peeked.value().first(tx_len);
        }

        if (prefix.size > static_cast<size_t>(-1) - first.size) {
            return m5::stl::make_unexpected(::m5::hal::v2::error::error_t::INVALID_ARGUMENT);
        }
        auto reserved = reserve(prefix.size + first.size);
        if (!reserved.has_value()) {
            return m5::stl::make_unexpected(reserved.error());
        }

        auto appended = append(prefix);
        if (!appended.has_value()) {
            return m5::stl::make_unexpected(appended.error());
        }
        appended = append(first);
        if (!appended.has_value()) {
            return m5::stl::make_unexpected(appended.error());
        }
        if (src == nullptr) {
            return {};
        }
        size_t remaining = tx_len;
        if (first.size > 0) {
            auto adv = src->advance(first.size);
            if (!adv.has_value()) {
                return m5::stl::make_unexpected(adv.error());
            }
            remaining -= first.size;
        }

        while (remaining > 0 && !src->eof()) {
            auto peeked = src->peek(remaining);
            if (!peeked.has_value()) {
                return m5::stl::make_unexpected(peeked.error());
            }
            auto span = peeked.value().first(remaining);
            if (span.size == 0) {
                break;
            }
            auto appended = append(span);
            if (!appended.has_value()) {
                return m5::stl::make_unexpected(appended.error());
            }
            auto adv = src->advance(span.size);
            if (!adv.has_value()) {
                return m5::stl::make_unexpected(adv.error());
            }
            remaining -= span.size;
        }
        return {};
    }

    uint8_t* data() const
    {
        return static_cast<uint8_t*>(_buffer.data());
    }

    size_t size() const
    {
        return _size;
    }

    bool empty() const
    {
        return _size == 0;
    }

private:
    ::m5::hal::v2::result_t<void> append(::m5::hal::v2::data::ConstDataSpan span)
    {
        if (span.size == 0) {
            return {};
        }
        if (span.data == nullptr) {
            return m5::stl::make_unexpected(::m5::hal::v2::error::error_t::INVALID_ARGUMENT);
        }
        if (span.size > static_cast<size_t>(-1) - _size) {
            return m5::stl::make_unexpected(::m5::hal::v2::error::error_t::INVALID_ARGUMENT);
        }
        auto reserved = reserve(_size + span.size);
        if (!reserved.has_value()) {
            return m5::stl::make_unexpected(reserved.error());
        }
        std::memcpy(data() + _size, span.data, span.size);
        _size += span.size;
        return {};
    }

    ::m5::hal::v2::result_t<void> reserve(size_t required)
    {
        if (required <= _capacity) {
            return {};
        }

        const size_t block_size = ::m5::hal::v2::memory::Allocator::tempBlockSize();
        size_t new_capacity     = std::max(roundUp(required, block_size), block_size);
        if (_capacity > 0) {
            const size_t doubled = (_capacity > (static_cast<size_t>(-1) / 2u)) ? required : (_capacity * 2u);
            new_capacity         = std::max(new_capacity, doubled);
        }

        if (!_buffer) {
            _buffer = ::m5::hal::v2::memory::TempBuffer{::m5::hal::v2::memory::defaultAllocator(), new_capacity};
            if (!_buffer) {
                return m5::stl::make_unexpected(::m5::hal::v2::error::error_t::OUT_OF_RESOURCE);
            }
        } else if (!_buffer.reallocate(new_capacity)) {
            return m5::stl::make_unexpected(::m5::hal::v2::error::error_t::OUT_OF_RESOURCE);
        }
        _capacity = new_capacity;
        return {};
    }

    static size_t roundUp(size_t value, size_t alignment)
    {
        if (value > static_cast<size_t>(-1) - (alignment - 1u)) {
            return value;
        }
        return ((value + alignment - 1u) / alignment) * alignment;
    }

    ::m5::hal::v2::memory::TempBuffer _buffer;
    size_t _size     = 0;
    size_t _capacity = 0;
};

}  // namespace m5::variants::frameworks::espidf::hal::v2::i2c::detail

#endif  // M5_HAL_VARIANTS_FRAMEWORKS_ESPIDF_HAL_I2C_BACKEND_MASTER_WRITE_BUFFER_INL
