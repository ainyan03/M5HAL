...
//
// The parent namespace is `m5::hal::v2::data`. Following the file/namespace
...
struct ConstDataSpan {
    ...
    /*! @brief Subrange from `offset`, up to `count` bytes (offset and count both clamped). */
    constexpr ConstDataSpan subspan(size_t offset, size_t count) const
    {
...
struct DataSpan {
    ...
    /*! @brief Subrange from `offset`, up to `count` bytes (offset and count both clamped). */
    constexpr DataSpan subspan(size_t offset, size_t count) const
    {
        size_t off = offset < size ? offset : size;
        size_t rem = size - off;
        return {data + off, count < rem ? count : rem};
...
/*!
  @brief Abstract base for a streaming data Source (producer side: pulled via peek/advance).
...
public:
    ...
    virtual m5::hal::v2::result_t<ConstDataSpan> peek(size_t max_len) = 0;
    virtual m5::hal::v2::result_t<void> advance(size_t N)             = 0;
...
/*!
  @brief Abstract base for a streaming data Sink (consumer side: pushed via reserve/commit).
