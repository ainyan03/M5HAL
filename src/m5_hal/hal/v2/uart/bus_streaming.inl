...
result_t<size_t> Bus_streaming::writeBackend(bus::OperationContext<AccessConfig>& context, data::Source* src,
                                             size_t len)
{
...
result_t<size_t> Bus_streaming::readBackend(bus::OperationContext<AccessConfig>& context, data::Sink* dst, size_t len)
{
    const auto& cfg = context.config;
    size_t done     = 0;
...
result_t<size_t> Bus_streaming::readableBytesBackend(bus::OperationContext<AccessConfig>& context)
{
    (void)context;
