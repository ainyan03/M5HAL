...
public:
    ...
    /*! @brief Convenience overload over a raw `(ptr, len)` pair. */
    MemorySource(const uint8_t* ptr, size_t len) : MemorySource{ConstDataSpan{ptr, len}}
    {
    }
...
public:
    ...
    /*! @brief Convenience overload over a raw `(ptr, len)` pair. */
    MemorySink(uint8_t* ptr, size_t len) : MemorySink{DataSpan{ptr, len}}
    {
    }
    ...
    {
        size_t remain = (_cursor < _buf.size) ? (_buf.size - _cursor) : 0;
        // Committing more than was reserved is a contract violation; catch it
        // in debug builds. Release still clamps so the cursor cannot overrun.
        M5HAL_ASSERT(N <= remain, "MemorySink::commit(N) exceeds the remaining capacity");
        _cursor += std::min(N, remain);
        return {};
    }
    ...
    /*! @brief Total backing-buffer size. */
    size_t capacity() const
    {
        return _buf.size;
    }

    /*! @brief Bytes still free before the buffer is full (`capacity() - written()`). */
    size_t remaining() const
    {
        return (_cursor < _buf.size) ? (_buf.size - _cursor) : 0;
    }
