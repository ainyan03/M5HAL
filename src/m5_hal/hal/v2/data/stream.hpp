...
// (§Stream adapters). The parent namespace `m5::hal::v2::data` is
...
struct StreamReader {
    ...
    virtual m5::hal::v2::result_t<size_t> read(DataSpan dst)  = 0;
    virtual m5::hal::v2::result_t<size_t> readableBytes(void) = 0;
};
...
public:
    StreamSource(StreamReader& reader, DataSpan scratch);
    StreamSource(StreamReader* reader, DataSpan scratch);
...
private:
    ...
    m5::hal::v2::result_t<void> drainSkip(bool blocking);
...
public:
    StreamSink(StreamWriter& writer, DataSpan scratch);
    StreamSink(StreamWriter* writer, DataSpan scratch);
