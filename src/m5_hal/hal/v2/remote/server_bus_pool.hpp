...
// arduino-esp32 only (see server_bus_pool.inl's HAS_ARDUINO_PROVIDER_):
...
struct ServerPhysicalBusPool {
    ...
    PhysPDMSlot pdm[kServerBusPoolSlots];
...
struct PDMSlot {
    ...
    uint8_t bus_id = 0;
    int8_t phys    = -1;
    pdm::AccessConfig acc_cfg;
    alignas(pdm::RxAccessor) uint8_t rx_buf[sizeof(pdm::RxAccessor)];
    pdm::RxAccessor* rx = nullptr;
};
...
struct ServerBusPool {
    ...
    PDMSlot pdm[kServerBusPoolSlots];
