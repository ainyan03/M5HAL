    ...
    const m5::hal::bus::AccessConfig& access_config)
{
    ...
    auto result = new SoftwareI2CMasterAccessor(*this, static_cast<const I2CMasterAccessConfig&>(access_config));
...
m5::stl::expected<void, m5::hal::error::error_t> I2CMasterAccessor::sendDummyClockWhileSdaLow(size_t count)
{
    ...
    if (!sda->read()) {
        ...
        constexpr uint32_t delay_cycle = DELAY_CYCLE;  //_nop_wait_w >> 1;
...
m5::stl::expected<size_t, m5::hal::error::error_t> SoftwareI2CMasterAccessor::write(const uint8_t* data, size_t len)
{
    ...
    /// @TODO:ディレイサイクル数を求める仕組みを作っておくこと。
    constexpr uint32_t delay_cycle = DELAY_CYCLE;  //_nop_wait_w >> 1;
    uint32_t dc0                   = delay_cycle >> 2;
    uint32_t dc1                   = delay_cycle - dc0;
    size_t result                  = 0;
    if (len == 0) {
    ...
    do {
        ...
        if (!scl->read()) {  // タイムアウト時間までクロックストレッチが解除されるのを待つ;
            auto stretch = waitClockStretch(static_cast<uint_fast16_t>(ac.timeout_msec));
            if (!stretch) {
                M5_LIB_LOGE("TIMEOUT");
                sda->writeHigh();
                                                                                                   ...
                                                                                                   bool last_nack)
{
    ...
    /// @TODO:ディレイサイクル数を求める仕組みを作っておくこと。
    constexpr uint32_t delay_cycle = DELAY_CYCLE;  //_nop_wait_w;
    uint32_t dc0                   = delay_cycle >> 1;
    uint32_t dc1                   = delay_cycle - dc0;
    size_t result                  = 0;
    do {
        // 最初の1bitの受信
        ...
        // クロックストレッチの判定と待機;
        if (!scl->read()) {  // タイムアウト時間までクロックストレッチが解除されるのを待つ;
            if (!waitClockStretch(static_cast<uint_fast16_t>(ac.timeout_msec))) {
                return m5::stl::make_unexpected(error::error_t::TIMEOUT_ERROR);
            }
        }
