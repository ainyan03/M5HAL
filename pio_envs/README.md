...
- `v2/examples.ini`: public v2 examples for users

## CLI-only env groups (`*.ini.cli`)

- `v0/check.ini.cli`: v0 header/build compatibility checks
- `v0v2/check.ini.cli`: v0+v2 same-TU coexistence checks on device targets
- `v2/check.ini.cli`: v2 header/build compatibility and shared API-surface checks
- `v2/test.ini.cli`: native and embedded test envs
- `v2/hil.ini.cli`: hardware-in-the-loop (HIL) test envs (require two devices with cross-wiring)
...
```bash
M5HAL_PIO_EXTRA_CONFIG=pio_envs/v2/check.ini.cli pio run -e v2_check_native
# a glob loads several at once:
M5HAL_PIO_EXTRA_CONFIG='pio_envs/*/check.ini.cli' pio run -e v0_check_native -e v2_check_native
```
...
The v2 check envs use `test/v2/build_check/build_check.hpp` as the common
...
are intentionally committed as `v2/examples.ini`.
