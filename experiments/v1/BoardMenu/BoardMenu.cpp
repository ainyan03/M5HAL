...
using SoftwareI2CBus = ::m5::hal::v1::i2c::variant::software::Bus;
#if M5HAL_FRAMEWORK_HAS_ARDUINO
using ArduinoSPIBus = ::m5::hal::v1::spi::variant::arduino::Bus;
#endif
using SoftwareSPIBus = ::m5::hal::v1::spi::variant::software::Bus;
#if M5HAL_ESPIDF_I2C_HAS_MASTER
using ESPIDFI2CBus = ::m5::hal::v1::i2c::variant::espidf::Bus;
#endif
...
enum class I2CBusVariant : uint8_t {
#if M5HAL_FRAMEWORK_HAS_ARDUINO
    Arduino,
#endif
    Software,
...
enum class SPIBusVariant : uint8_t {
#if M5HAL_FRAMEWORK_HAS_ARDUINO
    Arduino,
#endif
    Software,
...
#if M5HAL_FRAMEWORK_HAS_ARDUINO
I2CBusVariant current_i2c_variant = I2CBusVariant::Arduino;
#elif M5HAL_ESPIDF_I2C_HAS_MASTER
I2CBusVariant current_i2c_variant = I2CBusVariant::ESPIDF;
#else
I2CBusVariant current_i2c_variant = I2CBusVariant::Software;
#endif
SPIBusVariant current_spi_variant = SPIBusVariant::Software;

const char* i2cVariantName(I2CBusVariant variant)
{
    switch (variant) {
#if M5HAL_FRAMEWORK_HAS_ARDUINO
        case I2CBusVariant::Arduino:
            return "Arduino";
#endif
        case I2CBusVariant::Software:
            return "Software";
#if M5HAL_ESPIDF_I2C_HAS_MASTER
        case I2CBusVariant::ESPIDF:
            return "ESP-IDF";
#endif
    }
    return "?";
...
const char* spiVariantName(SPIBusVariant variant)
{
    switch (variant) {
#if M5HAL_FRAMEWORK_HAS_ARDUINO
        case SPIBusVariant::Arduino:
            return "Arduino";
#endif
        case SPIBusVariant::Software:
            return "Software";
#if M5HAL_ESPIDF_SPI_HAS_MASTER
        case SPIBusVariant::ESPIDF:
            return "ESP-IDF";
#endif
    }
    return "?";
...
{
    m5hal::i2c::I2CBusConfig cfg{PIN_I2C_SCL, PIN_I2C_SDA};
...
#if M5HAL_FRAMEWORK_HAS_ARDUINO
bool initI2CBus(m5hal::i2c::variant::arduino::Bus& bus)
{
    m5hal::i2c::variant::arduino::BusConfig cfg{&Wire, PIN_I2C_SCL, PIN_I2C_SDA};
    auto result = bus.init(cfg);
    if (!result.has_value()) {
        printResult("I2C bus init", result.error());
        return false;
...
{
    m5hal::i2c::variant::espidf::BusConfig cfg{PIN_I2C_SCL, PIN_I2C_SDA};
    auto result = bus.init(cfg);
    if (!result.has_value()) {
        printResult("I2C bus init", result.error());
        return false;
...
{
    current_i2c_variant = I2CBusVariant::Arduino;
    Serial.println("\n[I2C bus] Arduino");
}
#endif
...
{
    current_i2c_variant = I2CBusVariant::Software;
    Serial.println("\n[I2C bus] Software");
}
...
{
    current_i2c_variant = I2CBusVariant::ESPIDF;
    Serial.println("\n[I2C bus] ESP-IDF");
}
#endif

uint16_t scanI2C(m5hal::i2c::I2CBus& bus, const char* label)
{
...
void withSelectedI2CBus(Action action)
{
...
void runI2CScan()
{
    withSelectedI2CBus([](m5hal::i2c::I2CBus& bus) {
        char label[32];
...
void runI2CRegisterRead()
{
    withSelectedI2CBus([](m5hal::i2c::I2CBus& bus) {
        ...
        cfg.freq       = I2C_FREQ;
        cfg.timeout_ms = 100;
        m5hal::i2c::I2CMasterAccessor dev{bus, cfg};
...
void configureSPIAccess(m5hal::spi::SPIMasterAccessConfig& access, int cs_pin, uint32_t freq, uint8_t write_dummy_cycle)
{
    access.pin_cs                = cs_pin;
    access.freq                  = freq;
    access.timeout_ms            = 100;
    access.spi_mode              = 0;
    access.spi_order             = 0;
...
template <typename Bus>
bool initSPIBus(Bus& bus, m5hal::spi::SPIMasterAccessConfig& access, int dc_pin, int cs_pin, uint32_t freq,
                uint8_t write_dummy_cycle)
{
    setOutputHigh(cs_pin);

    m5hal::spi::SPIBusConfig bus_cfg;
...
#if M5HAL_FRAMEWORK_HAS_ARDUINO
bool initSPIBus(ArduinoSPIBus& bus, m5hal::spi::SPIMasterAccessConfig& access, int dc_pin, int cs_pin, uint32_t freq,
                uint8_t write_dummy_cycle)
{
    setOutputHigh(cs_pin);

    m5hal::spi::variant::arduino::BusConfig bus_cfg;
    bus_cfg.spi      = &SPI;
    bus_cfg.pin_clk  = PIN_SPI_CLK;
    bus_cfg.pin_mosi = PIN_SPI_MOSI;
    bus_cfg.pin_miso = PIN_SPI_MISO;
...
#if M5HAL_ESPIDF_SPI_HAS_MASTER
bool initSPIBus(ESPIDFSPIBus& bus, m5hal::spi::SPIMasterAccessConfig& access, int dc_pin, int cs_pin, uint32_t freq,
                uint8_t write_dummy_cycle)
{
    setOutputHigh(cs_pin);

    m5hal::spi::variant::espidf::BusConfig bus_cfg;
    bus_cfg.pin_clk  = PIN_SPI_CLK;
    bus_cfg.pin_mosi = PIN_SPI_MOSI;
    bus_cfg.pin_miso = PIN_SPI_MISO;
    bus_cfg.pin_dc   = dc_pin;
...
{
    current_spi_variant = SPIBusVariant::Arduino;
    Serial.println("\n[SPI bus] Arduino");
}
#endif
...
{
    current_spi_variant = SPIBusVariant::Software;
    Serial.println("\n[SPI bus] Software");
}
...
{
    current_spi_variant = SPIBusVariant::ESPIDF;
    Serial.println("\n[SPI bus] ESP-IDF");
}
#endif

template <typename Action>
void withSelectedSPIBus(m5hal::spi::SPIMasterAccessConfig& access, int dc_pin, int cs_pin, uint32_t freq, Action action)
{
...
void runSPIWrite()
{
    ...
    m5hal::spi::SPIMasterAccessConfig access;
    withSelectedSPIBus(access, PIN_SPI_DC, PIN_SPI_CS, SPI_FREQ, [&](m5hal::spi::SPIBus& bus) {
        m5hal::spi::SPIMasterAccessor spi{bus, access};
...
void configureLcdAccess(m5hal::spi::SPIMasterAccessConfig& access)
{
...
bool lcdWriteCommand(m5hal::spi::SPIMasterAccessor& lcd, uint32_t command, const char* label)
{
...
bool lcdInitSequence(m5hal::spi::SPIMasterAccessor& lcd)
{
    uint8_t colmod = 0x55;
...
bool lcdSetWindow(m5hal::spi::SPIMasterAccessor& lcd, int x, int y, int w, int h)
{
...
bool lcdDrawTile(m5hal::spi::SPIMasterAccessor& lcd, int bx, int by)
{
...
void runLcdInit()
{
    ...
    m5hal::spi::SPIMasterAccessConfig access;
    withSelectedSPIBus(access, PIN_LCD_DC, PIN_LCD_CS, LCD_SPI_FREQ, [&](m5hal::spi::SPIBus& bus) {
        configureLcdAccess(access);
        m5hal::spi::SPIMasterAccessor lcd{bus, access};
