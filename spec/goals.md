...
> **旧称**: 本文書は 2026-07 まで新 API 世代を `v1`、 移行期リリース系列を
> `v0.2.x` と呼んでいた。 2026-07 の呼称再編 (ADR 047) でそれぞれ `v2` /
> `1.x.x` に改めた。 過去の ADR (001〜046) 等の歴史文書は旧称のまま残す。
...
4. **API 安定期間** — `m5::hal::v2::*` の公開 API に利用者影響のある breaking change が一定期間発生していない
