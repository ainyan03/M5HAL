...
| examples build | `examples/v1/HowToUse/I2C` / `SPI` / `UART` / `UARTEcho` の ESP32 / ESP32-S3 ビルド + `Bytecode` / `RemoteBus` (Core BASIC 前提、ESP32 のみ) + `RemoteBus/host` (PC 側、native) | `pio run -e HowToUse_I2C_esp32 -e HowToUse_I2C_esp32s3 -e HowToUse_SPI_esp32 -e HowToUse_SPI_esp32s3 -e HowToUse_UART_esp32 -e HowToUse_UART_esp32s3 -e HowToUse_UARTEcho_esp32 -e HowToUse_UARTEcho_esp32s3 -e HowToUse_Bytecode_esp32 -e HowToUse_RemoteBus_esp32 -e HowToUse_RemoteBusHost_native` |
