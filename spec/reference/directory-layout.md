...
例外として `_macro/offer_all.inl` / `_macro/offer_kind.inl` / `_macro/offer_runtime_only.inl` はマクロ展開の都合で 1 行ネスト形式を許容する。
...
- `src/m5_hal/variants/platforms/_checker.hpp` — `M5HAL_V1_TARGET_PLATFORM_VARIANT_ID` / `_PATH` の検出 (識別番号の正本は `variants/ids.hpp`。 無印は凍結 v0 が所有)
