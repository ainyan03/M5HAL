...
- variant 内の HAL 提供物は `namespace hal { ... }` に置く
- variant 内では必要に応じて `using namespace ::m5::hal;` を置いて unqualified 参照を救済する
- `types::*` は `::m5::hal::types::*` のように fully qualified で書く
...
- `src/m5_hal/variants/platforms/_checker.hpp` — `M5HAL_PLATFORM_NUMBER_*`, `M5HAL_TARGET_PLATFORM_*` 系
