# design/runtime — runtime 設備 (time + mutex + task) の variant 注入

> **読者**: 実装者・レビュー向け（設計仕様）。

実行環境依存の基礎設備 (時刻取得・遅延・mutex・task) を **runtime kind** として variant が
供給する。 runtime kind は 3 つの独立した **sub-kind** に分割されており、 sub-kind ごとに
異なる variant が勝者になれる:
...
注入を維持する** ([variants.md](variants.md) §offer 要件の例外)。 Mutex / Task は型 alias。
...
| variant | RUNTIME (time) | RUNTIME_MUTEX | RUNTIME_TASK |
|---|---|---|---|
| `frameworks/freertos` | 申告しない | FreeRTOS mutex (`xSemaphoreCreateMutex`) | FreeRTOS task (`xTaskCreatePinnedToCore`) |
| `frameworks/arduino` | `::millis` / `::micros` / `::delay` / `::delayMicroseconds` | 申告しない | 申告しない |
| `frameworks/espidf` | `esp_timer_get_time` (+ `vTaskDelay` / `esp_rom_delay_us`) | 申告しない | 申告しない |
| `frameworks/posix` | `clock_gettime(CLOCK_MONOTONIC)` / `nanosleep` | `std::timed_mutex` | `std::thread` wrapper |
| `frameworks/stub` | 単調フェイク (native テストで決定的) | シングルタスク owner ガード | no-op Task |
| `frameworks/software` | 申告しない (bus 実装 variant) | 同左 | 同左 |

- **FreeRTOS variant** (`variants/frameworks/freertos/`) は独立 framework variant として offer
  機構にフル参加する。 scan 順で arduino / espidf より前に配置されるため RUNTIME_MUTEX と
  RUNTIME_TASK を先に勝ち取る。 time functions は申告しない (framework 固有 API に依存するため
  arduino / espidf に委ねる)。 現在の FreeRTOS 検出は ESP-IDF のインクルードレイアウト
...
- scan 順 / first-hit / `M5HAL_V2_SELECTED_VARIANT_RUNTIME{,_MUTEX,_TASK}` マーカは既存 kind
  と同一規約 ([variants.md](variants.md))。 **NONE fallback は無い**: stub が 3 sub-kind 全てを
  常に申告するため選択は必ず成立し、 不成立は `hal/v2/runtime/runtime.hpp` の `#error` で止める
  (`bus::IBus` が型として必要なので NONE を許容できない)。

## early scan (bus kind との違い)

`bus::IBus` は `runtime::Mutex` を **値で内蔵** するため、 runtime の勝者選択 (3 sub-kind 全て)
...
1. framework `_checker.hpp` で検出済みの variant を本 scan と同じ順 (**freertos → arduino →
   ...
   RUNTIME_TASK を先に勝ち取り、 arduino / espidf が RUNTIME (time) を勝つ
