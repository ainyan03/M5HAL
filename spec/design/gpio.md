...
- **restore 安全性**: `restore()` は直近の `backup()` が実際に MCU ピン状態を退避できた場合だけレジスタを
  書き戻す。 `backup()` 未実行、 invalid / expander pin、 範囲外 pin は no-op のまま。 `captured()` で
  復元可能な退避状態を持っているか確認できる。 `setPin()` は古い退避状態を破棄する
...
- **`dismiss()`**: 復元を取り消す（新しいピン設定をそのまま残したい場合）。 `armed()` で復元保留中か確認できる。
  invalid / expander pin のように退避が no-op だった場合は最初から disarm される。 `captured()` は内包する
  `PinBackup` が復元可能な退避状態を持つかを返す
