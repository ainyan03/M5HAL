...
- **subscribe は命令単位で all-or-nothing**: 列挙ピンのどれかが失敗したら命令全体が不発になり、
  購読状態は命令前から変化しない (先頭側だけ購読が残る部分適用はしない)
...
- **device 側の実行予算** (`Server::Config`): script 1 本あたりの `delay_ms` 合計 (`max_delay_ms`)、`bus_configure` が運ぶタイムアウト上限 (`max_bus_timeout_ms`)、`bus_transfer` が要求できる rx 確保量の上限 (`max_transfer_rx` — wire の LenVar は u32 を綴れるため、無制限だと 1 メッセージで RAM を占有できてしまう)。違反は実行前の prescan が `report_error(INVALID_ARGUMENT)` で拒否する
- **host 側の受信制限**: `RemoteSession` の runner は受信専用 (`BytecodeRunner::setReceiveOnly`)。応答 / イベント script が実行できるのは `store_data` / `report_*` / `evt_*` のみで、実行系 opcode (`delay_ms`、`bus_*`、`gpio_*`) は `PROTOCOL_ERROR` で拒否する — 異常な server の 1 メッセージが host を停止・操作することはない
- **イベントは要求状態に触れない**: イベント script は `runEvent` で実行され、直前の応答の store slot / report 状態を消去も上書きもしない (`poll()` を挟んでも `storedData()` は有効)
