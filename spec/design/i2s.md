...
struct TxAccessor : bus::IAccessor, data::StreamWriter;  // 再生 = StreamWriter としても振る舞う
```

- `TxAccessor` は [data_io.md](data_io.md) の `StreamWriter` を実装する。汎用ストリーム
  コードがそのまま音声出力先として使える。
- RX (録音) は将来拡張。`IBusConfig::pin_din` のみ予約済みで、accessor / read API は
...
| 構造体 | 内容 | 所有 |
|---|---|---|
| `IBusConfig` | pins (bclk / ws / dout / din / mclk)、`tx_buffer_size` (DMA 総量の目安) | bus を生成・登録する側 (device) |
| `AccessConfig` | `sample_rate_hz` / `bits_per_sample` (当面 16) / `channels` (1=mono, 2=stereo) / `timeout_ms` (bus lock) / `write_timeout_ms` | アクセスする側 |
...
- `writableBytes()` は「いま write してもブロックしない量」。送信側のフロー制御の源泉で、
  ...
  追跡をそのまま credit に使う。
- **戻り値の切れ目契約** (Audio 層実装者が最初に必要とする前提):
