...
- **名前 hash を ID 正本にしない**: constexpr hash (`mmh3` 等) は preprocessor の `#if` で使えず、
  32bit で u16 と不整合、値からグループが読めない。予約域 250〜65535 は out-of-tree variant が
  名前 hash 由来の値で名乗る分散拡張の将来余地として温存する (u16 固定はこれと整合する)。
