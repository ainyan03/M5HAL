...
| 軸 | 性質 | 例 |
|---|---|---|
| **platform variant** | MCU や ホスト OS の物理層に直接依存。 ビルドごとに 1 つだけ選ばれる | `espressif/esp32`, `espressif/esp32s3`, ... |
| **framework variant** | ソフトウェア抽象層 (Arduino SDK, ESP-IDF, FreeRTOS, POSIX, ソフト実装、 リモート等) に依存。 複数共存可能 | `arduino`, `espidf` (将来), `software`, `stub` |
| **stub variant** | 常時末尾フォールバック。 全 HAL の no-op 具象を提供。 native ビルドでは stub が flat 注入される | `frameworks/stub/` |
...
src/m5_hal/variants/
  frameworks/
    ...
    stub/
      _offer.hpp              capability 全申告
      hal.hpp                 no-op 具象 (inline 定義のみ、 hal.inl 不要)
      hal/gpio/gpio.hpp
  platforms/
    _checker.hpp          platform 軸の検出 (M5HAL_PLATFORM_NUMBER_*, M5HAL_TARGET_PLATFORM_*)
    espressif/
      esp32/                chip-family 単位 (ESP32 / S2 / S3 / C3 / C6 / P4 等を統合)
        _offer.hpp
...
- `src/m5_hal/variants/frameworks/arduino/hal/i2c/i2c.hpp` ⇔ `m5::variants::frameworks::arduino::hal::i2c::*`
- `src/m5_hal/variants/platforms/espressif/esp32/hal/gpio/gpio.hpp` ⇔ `m5::variants::platforms::espressif::esp32::hal::gpio::*`
...
kind 別ファイルは自己完結 (include guard、 namespace スキャフォールド、 `ARDUINO` 等のガード、 `using namespace ::m5::hal;` 救済を各ファイルが持つ)。
...
2. 提供 HAL ごとに 3 種類の alias を生成 (ソース namespace は `::m5::<BASE_NS>::hal::<kind>`):
...
| マクロ | 用途 |
|---|---|
| `M5HAL_TARGET_PLATFORM_NUMBER` | MCU 識別整数 (`0` は不明 = native ビルド) |
| `M5HAL_TARGET_PLATFORM_PATH` | variant ヘッダの動的 include パス (例: `m5_hal/variants/platforms/espressif/esp32`) |
| `M5HAL_FRAMEWORK_HAS_<NAME>` | framework 検出フラグ (例: `M5HAL_FRAMEWORK_HAS_ARDUINO`) |
...
```
1. platform variant      (M5HAL_TARGET_PLATFORM_PATH 経由で動的 include)
...
「最初に申告した variant」 が flat 注入される。 stub は全 HAL を申告するが、 上位 variant が先に注入した HAL では fallback として控える。

posix は素の POSIX host (Arduino / ESP-IDF SDK を含まないビルド) で **UART のみ** を申告し、 host serial を既定の UART provider にする (host の UART スロットは他の host variant が埋めないため)。 これは **opt-out**: 既定で有効、 `M5HAL_DISABLE_POSIX` 定義で抑止する (host で UART を意図的に未提供へ戻したいテスト等)。 stub は host 実装を持たない HAL kind の no-op を引き続き担う。
...
| frameworks | `stub/` | 全 HAL の no-op fallback (test 用) |
| frameworks | `software/` | software bit-bang による I2C 等の framework 中立具象 |
| frameworks | `espidf/` | ESP-IDF framework の HAL 具象 |
| frameworks | `posix/` | POSIX host の UART 具象 (termios serial)。 opt-out (既定有効、 `M5HAL_DISABLE_POSIX` で抑止) |
| platforms | `espressif/esp32/` | ESP32 chip-family (ESP32 / S3 / C6 / P4) のレジスタ直叩き具象 |
