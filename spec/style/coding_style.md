...
- variant の**公開型** (`Bus_<variant>` / `BusConfig_<variant>` / gpio の `Port_<variant>` 等) は `m5::hal::v1::<kind>` 直下に variant suffix 付きで定義する ([../design/variants.md](../design/variants.md) §offer 要件。 ディレクトリ階層 1:1 規約の明示的例外 — [../architecture.md](../architecture.md) §ディレクトリと namespace)
- variant の**内部構造** (service 群・レジスタ層・固有ユーティリティ) は `m5::variants::{frameworks,platforms}::<name>[::<chip>]::hal::v1::*` 配下でディレクトリ階層と一致させる (例: `software/hal/i2c/i2c.hpp` の detail 群は `m5::variants::frameworks::software::hal::v1::i2c::detail::*`)
- variants namespace 側のブロック内では必要に応じて `using namespace ::m5::hal::v1;` を置ける (kind 直下に定義する公開型は enclosing namespace 解決で足りるため不要)
