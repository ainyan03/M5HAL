...
// Skeleton tests for the `m5::hal::v2::gpio::GPIO` abstract and the
...
using StubGPIO = ::m5::hal::v2::gpio::GPIO_stub;
...
{
    auto* a = ::m5::hal::v2::gpio::getGPIO_stub();
    auto* b = ::m5::hal::v2::gpio::getGPIO_stub();
    EXPECT_EQ(a, b);
    EXPECT_NE(a, nullptr);
}
...
{
    // Flat injection makes `m5::hal::v2::gpio::GPIO` an alias for
    // `stub::GPIO` when the stub variant is active. In native tests,
    // the stub (with `HAS_HAL_GPIO_`) is the last to be scanned, so
    // `m5::hal::v2::gpio::GPIO` can construct the concrete directly.
    ::m5::hal::v2::gpio::GPIO concrete;
    ::m5::hal::v2::gpio::IGPIO* abstract_ptr = &concrete;
