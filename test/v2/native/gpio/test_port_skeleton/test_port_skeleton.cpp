...
// (`m5::hal::v2::gpio::Port_stub`) drives
...
TEST(PortSkeleton, AllBitsIndependent)
{
    ...
    for (uint8_t i = 0; i < 32; ++i) {
        port.write(static_cast<::m5::hal::v2::types::gpio_local_pin_t>(i), true);
    }
    for (uint8_t i = 0; i < 32; ++i) {
        EXPECT_TRUE(port.read(static_cast<::m5::hal::v2::types::gpio_local_pin_t>(i))) << "bit " << static_cast<int>(i);
    }
    for (uint8_t i = 0; i < 32; ++i) {
        port.write(static_cast<::m5::hal::v2::types::gpio_local_pin_t>(i), false);
    }
    for (uint8_t i = 0; i < 32; ++i) {
        EXPECT_FALSE(port.read(static_cast<::m5::hal::v2::types::gpio_local_pin_t>(i)))
