...
using m5::hal::v2::data::LimitedSink;
using m5::hal::v2::data::LimitedSource;
...
// ============================================================================
// U6: ptr+len ctor equivalence
// MemorySource(const uint8_t*, size_t) and MemorySink(uint8_t*, size_t)
// delegate to the span ctor and must behave identically.
// ============================================================================

TEST(MemorySource, PtrLenCtorMatchesSpanCtor)
{
    const std::array<uint8_t, 4> bytes{0xA0, 0xA1, 0xA2, 0xA3};
    MemorySource span_src{ConstDataSpan{bytes.data(), bytes.size()}};
    MemorySource ptr_src{bytes.data(), bytes.size()};

    // Both must report the same peek results.
    auto span_peek = span_src.peek(3);
    auto ptr_peek  = ptr_src.peek(3);
    ASSERT_TRUE(span_peek);
    ASSERT_TRUE(ptr_peek);
    EXPECT_EQ(span_peek->size, ptr_peek->size);
    EXPECT_EQ(span_peek->data[0], ptr_peek->data[0]);
    EXPECT_EQ(span_peek->data[2], ptr_peek->data[2]);

    // After an equal advance, remaining peek must also match.
    ASSERT_TRUE(span_src.advance(2));
    ASSERT_TRUE(ptr_src.advance(2));
    auto span_rest = span_src.peek(100);
    auto ptr_rest  = ptr_src.peek(100);
    ASSERT_TRUE(span_rest);
    ASSERT_TRUE(ptr_rest);
    EXPECT_EQ(span_rest->size, ptr_rest->size);
    EXPECT_EQ(span_rest->data[0], ptr_rest->data[0]);
}

TEST(MemorySink, PtrLenCtorMatchesSpanCtor)
{
    std::array<uint8_t, 4> span_buf{};
    std::array<uint8_t, 4> ptr_buf{};
    MemorySink span_sink{DataSpan{span_buf.data(), span_buf.size()}};
    MemorySink ptr_sink{ptr_buf.data(), ptr_buf.size()};

    // Both must offer the same reserved size.
    auto span_rsv = span_sink.reserve(3);
    auto ptr_rsv  = ptr_sink.reserve(3);
    ASSERT_TRUE(span_rsv);
    ASSERT_TRUE(ptr_rsv);
    EXPECT_EQ(span_rsv->size, ptr_rsv->size);

    // After an equal commit, written() must match.
    ASSERT_TRUE(span_sink.commit(3));
    ASSERT_TRUE(ptr_sink.commit(3));
    EXPECT_EQ(span_sink.written(), ptr_sink.written());
}

// ============================================================================
// U12: MemorySink::capacity() / remaining()
// ============================================================================

TEST(MemorySink, CapacityEqualsBackingSize)
{
    std::array<uint8_t, 7> buf{};
    MemorySink sink{DataSpan{buf.data(), buf.size()}};
    EXPECT_EQ(sink.capacity(), 7u);
}

TEST(MemorySink, InitialRemainingEqualsCapacity)
{
    std::array<uint8_t, 5> buf{};
    MemorySink sink{DataSpan{buf.data(), buf.size()}};
    EXPECT_EQ(sink.remaining(), sink.capacity());
    EXPECT_EQ(sink.remaining(), 5u);
}

TEST(MemorySink, RemainingDecreasesAfterCommitCapacityInvariant)
{
    std::array<uint8_t, 8> buf{};
    MemorySink sink{DataSpan{buf.data(), buf.size()}};
    const size_t cap = sink.capacity();  // 8

    ASSERT_TRUE(sink.commit(3));
    EXPECT_EQ(sink.capacity(), cap);        // capacity is invariant
    EXPECT_EQ(sink.remaining(), cap - 3u);  // 5

    ASSERT_TRUE(sink.commit(5));
    EXPECT_EQ(sink.capacity(), cap);  // still invariant
    EXPECT_EQ(sink.remaining(), 0u);
    EXPECT_TRUE(sink.closed());
}
