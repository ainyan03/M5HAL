...
public:
    ...
    ServicePoll serviceImpl(const ServiceContext& ctx) override
    {
        ...
        return ServicePoll{_result, next_due};
    }
...
TEST(ServiceRunner, SkipsServiceUntilNextDue)
{
    ...
    EXPECT_TRUE(runner.runOnce(ServiceContext{10}));
    EXPECT_EQ(log, (std::vector<int>{1, 2}));

    log.clear();
    EXPECT_TRUE(runner.runOnce(ServiceContext{99}));
    EXPECT_EQ(log, (std::vector<int>{2}));

    log.clear();
    timed.next_due = 0;
    EXPECT_TRUE(runner.runOnce(ServiceContext{100}));
    EXPECT_EQ(log, (std::vector<int>{1, 2}));
}
