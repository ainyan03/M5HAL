...
using SoftwareSpiBus = ::m5::hal::v2::spi::Bus_software;

// Init a caller-owned bus in place. A Bus embeds a runtime Mutex and is
...
::m5::hal::v2::spi::MasterAccessConfig makeAccessConfig(uint8_t mode = 0, uint8_t order = 0)
{
...
void testWriteCommandAddressDataWirePhases()
{
    ...
    ::m5::hal::v2::spi::MasterAccessor spi{bus, cfg};

    const uint8_t tx[] = {0xDE, 0xAD};
    cap::start();
    auto result = spi.writeCommandAddressData(0x02, 0x001234, ::m5::hal::v2::data::ConstDataSpan{tx, sizeof(tx)});
    cap::stop();
...
void testReadCommandAddressDataWirePhases()
{
    ...
    uint8_t rx[4]{};
    cap::start();
    auto result = spi.readCommandAddressData(0x0B, 0x001234, ::m5::hal::v2::data::DataSpan{rx, sizeof(rx)});
    cap::stop();
...
void testWriteUsesLsbFirstBitOrder()
{
    ...
    cap::start();
    auto result = spi.write(::m5::hal::v2::data::ConstDataSpan{tx, sizeof(tx)});
    cap::stop();
