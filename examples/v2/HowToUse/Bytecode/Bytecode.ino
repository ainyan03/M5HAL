...
constexpr uint8_t OP_DELAY_MS        = static_cast<uint8_t>(bytecode::OpCode::DelayMs);
constexpr uint8_t OP_BUS_CONFIGURE   = static_cast<uint8_t>(bytecode::OpCode::BusConfigure);
constexpr uint8_t OP_BUS_TRANSFER    = static_cast<uint8_t>(bytecode::OpCode::BusTransfer);
constexpr uint8_t OP_GPIO_SET_MODE   = static_cast<uint8_t>(bytecode::OpCode::GpioSetMode);
constexpr uint8_t OP_GPIO_WRITE_HIGH = static_cast<uint8_t>(bytecode::OpCode::GpioWriteHigh);
constexpr uint8_t OP_GPIO_WRITE_LOW  = static_cast<uint8_t>(bytecode::OpCode::GpioWriteLow);
...
constexpr uint8_t SPI_MODE_HALFDUPLEX_DC = static_cast<uint8_t>(m5hal::spi::spi_data_mode_t::HalfDuplexWithDcPin);
...
static constexpr uint8_t SCRIPT_GPIO_BLINK[] = {
    0x04,
    OP_GPIO_SET_MODE,
    BC_GPIO_OUTPUT,            // gpio_set_mode: Output,
...
static constexpr uint8_t SCRIPT_I2C_READ[] = {
    ...
    0x80,          0x1A,
    0x06,          0x00,  //   freq      = 400000
    0x32,          0x00,
    0x00,          0x00,             //   timeout   = 50 ms
...
static constexpr uint8_t SCRIPT_LCD_INIT[] = {
    ...
    BC_DELAY_MS(120),           //
    0x13,
    OP_BUS_CONFIGURE,  // bus_configure (payload 18)
    KIND_SPI,
    0x00,  //   kind=SPI, bus_id=0
    0x0E,
    0x00,  //   pin_cs = 14
    0x80,
    0x96,
    ...
    0x08,
    0x00,  //   command 8 bit, no address phase
    0x00,
    0x00,  //   no dummy cycles
    BC_LCD_CMD(0x01),
    BC_DELAY_MS(150),  // SWRESET
    BC_LCD_CMD(0x11),
    BC_DELAY_MS(120),  // SLPOUT
...
m5hal::i2c::MasterAccessConfig i2c_acc_cfg;  // overwritten by bus_configure scripts
m5hal::spi::MasterAccessConfig spi_acc_cfg;
m5hal::i2c::MasterAccessor i2c_dev{i2c_bus, i2c_acc_cfg};
m5hal::spi::MasterAccessor spi_dev{spi_bus, spi_acc_cfg};
